...
一种无需 context 的解决方案是将[ Avatar 组件自身传递下去](https://react.docschina.org/docs/composition-vs-inheritance.html#containment)，因而中间组件无需知道 user 或者 avatarSize 等 props：
...
示例，下述组件在 DevTools 中将显示为 MyDisplayName：示例，下述组件在 DevTools 中将显示为 MyDisplayName：
...
## 过时的 API
>注意\
>先前 React 使用实验性的 context API 运行，旧的 API 将会在所有 16.x 版本中得到支持，但用到它的应用应该迁移到新版本。过时的 API 将在未来的 React 版本中被移除。阅读[过时的 context 文档](https://react.docschina.org/docs/legacy-context.html)了解更多。